; Exemplo de utilização de processos com comunicação por LOCK (precisa de um circuito com teclado e displays)
;
...
; **********************************************************************
; TECLADO - Processo que lê uma tecla da última linha do teclado
;		  ATENÇÃO: Devido ao ciclo de atraso no programa principal,
;		  um simples toque rápido no teclado pode passar despercebido!
;
...
PROCESS SP_teclado			; declaração do processo
teclado:
...
duplica:
	...
	MOV	R4, [delta_contador]	; NÃO pode estar num registo (processos não partilham registos!)
	SHL	R4, 1				; duplica o delta (positivo ou negativo)
	MOV	[delta_contador], R4	; atualiza na memória
	JMP	duplica
